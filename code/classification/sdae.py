...
class CDAutoEncoder(nn.Module):
    ...
    def __init__(self, input_size, output_size, stride):
        ...
        self.forward_pass = nn.Sequential(
            nn.Conv1d(input_size, output_size, kernel_size=3, stride=2, padding=0),
            nn.ReLU(),
        )
        self.backward_pass = nn.Sequential(
            nn.ConvTranspose1d(output_size, input_size, kernel_size=3, stride=2, padding=0), 
            nn.ReLU(),
        )
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            print("X_recon: ",x_reconstruct.shape)
